// ---------------------------------------------------------------------------
// Рабочий кабинет: вход для операторов и менеджеров.
//
// Оператор видит свои чаты и свои заявки, отвечает клиентам и меняет статусы.
// Менеджер видит список своих операторов; выбрав оператора, читает его чаты
// и смотрит статистику по его заявкам. Писать менеджер не может — только читать.
// ---------------------------------------------------------------------------
...
const KEY_STORE = 'olan-cabinet-token';
...
export default function App() {
  ...
  const loadMe = async () => {
    if (!key) return;
    try { setMe(await getJson(`/api/operator/me?key=${encodeURIComponent(key)}`)); }
    catch {
      try { localStorage.removeItem(KEY_STORE); } catch { /* ignore */ }
    ...
    <div className="flex h-screen flex-col overflow-hidden bg-black text-white">
      <header className="flex shrink-0 flex-wrap items-center gap-2 border-b border-cyan-500/15 bg-slate-950 px-3 py-3 sm:px-5">
        <div className="bg-gradient-to-r from-cyan-400 to-blue-500 bg-clip-text text-base font-bold text-transparent sm:text-lg">
          OLAN<span className="hidden sm:inline"> — Кабинет</span>
        </div>
